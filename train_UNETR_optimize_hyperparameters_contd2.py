...
train_images_path = Path('/scratch/b/bojana/rozakmat/TBI/GT_filtered+raw/') #raw path images
...
train_labels_path = Path('/scratch/b/bojana/rozakmat/TBI/GT_filtered+raw/')#labels path
...
mouse_ids_path = Path('/scratch/b/bojana/rozakmat/TBI/raw')#each mouse has its own folder with raw data in it
...
model.load_state_dict(torch.load(
    os.path.join(directory, "best_metric_model_ctd.pth")))
        ...
        with torch.no_grad():
            ...
            if metric > best_metric:
                ...
                torch.save(model.state_dict(), os.path.join(
                    directory, "best_metric_model_ctd2.pth"))
...
df.to_csv(directory + '/metrics_loss_contd2.csv')
df = pd.DataFrame()
...
df.to_csv(directory + '/metrics_validation_round_3.csv')
...
plt.xlabel("epoch")
plt.plot(x, y)
plt.savefig(directory +'ipynb_trial_Val_Loss+Dice_contd3.png')
