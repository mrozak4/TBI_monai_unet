...
data_dicts = [
    {"image":image_name}
    for image_name in mouse_ids if not os.path.exists(re.sub('.tif','_pred.npy',re.sub('matt_raw_warped_single','/home/rozakmat/projects/rrg-bojana/rozakmat/TBI_monai_UNET/matt_raw_warped_single_upsampled',image_name)))
]
...
with torch.no_grad():
    for i, pred_data in tqdm(enumerate(pred_loader)):
        ...
        if not os.path.exists(re.sub('.tif','_pred.npy',new_file_name)):
            pred_array = np.empty((num_evals,3,512,512,254),dtype=np.float16)
            ...
            np.save(re.sub('projects/rrg-bojana/rozakmat','projects/rrg-bojana/rozakmat',re.sub('.tif','_mean.npy',new_file_name)),mean)
            print(re.sub('projects/rrg-bojana/rozakmat','projects/rrg-bojana/rozakmat',re.sub('.tif','_mean.npy',new_file_name)))
            np.save(re.sub('.tif','_pred.npy',new_file_name),pred_array)
            print(re.sub('.tif','_pred.npy',new_file_name))
