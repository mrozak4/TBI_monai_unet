...
from torch_geometric.nn import GraphSAGE, GraphUNet, GAT, GCN
from scipy.ndimage import distance_transform_edt
...
criterion = torch.nn.CrossEntropyLoss(weight = torch.tensor(compute_class_weight(class_weight='balanced',classes=np.unique(dataset_val.y.detach().numpy()),y=dataset_val.y.detach().numpy())**1.5).type(torch.float))
