...
train_transforms = Compose(
    [
        ...
        Spacingd(keys=["image", "label"],  pixdim=(
            1/2, 1/2, 0.375/2), mode=("bilinear", "nearest")),
...
train_ds = CacheDataset(
    data=train_files, transform=train_transforms, cache_rate=0.55, num_workers=4)
train_loader = DataLoader(train_ds, batch_size=params["batch_size"], shuffle=True, num_workers=4)

val_ds = CacheDataset(
    data=val_files, transform=val_transforms, cache_rate=0.55, num_workers=4)
val_loader = DataLoader(val_ds, batch_size=1, num_workers=4)

test_ds = CacheDataset(
    data=test_files, transform=val_transforms, cache_rate=0.55, num_workers=4)
...
random.seed(12)
max_epochs = 50#2400#800#params["max_epochs"]
val_interval = 2
best_metric = -1
best_metric_epoch = -1
epoch_loss_values = []
