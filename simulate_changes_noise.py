...
files = [x for x in files if not os.path.exists(re.sub('_warped.pickle','_changes_noise2.npz',re.sub('matt_raw_warped_single_upsampled_seg','simulated_changes',x)))]
...
for file in tqdm(files):
    if not os.path.exists(re.sub('_warped.pickle','_changes_noise2.npz',re.sub('matt_raw_warped_single_upsampled_seg','simulated_changes',file))):
        ...
        _sigma = []
        _std = []
        if len(graph.edges) < 1500 and len(graph.edges) > 20:
                            ...
                            for i in np.random.choice(range(len(graph.edges)),30):
                                ...
                                if len(path) > 40:
                                    ...
                                    def calc_fwhm_path(I):
                                        ...
                                        np.random.seed()
                                        sigma = np.random.uniform(0,500)
                                        std = np.std(points_img)
                                        points_img_noise =  points_img + np.random.normal(0,sigma,points_img.shape).astype(np.int16)
                                        points_img_no_smooth = np.copy(points_img)
                                        points_img = sp.ndimage.gaussian_filter(points_img, sigma = _pred_radii*.4)
                                        _rand = 1 #np.random.normal(1,0.1)
                                        points_img_noise_smooth = sp.ndimage.gaussian_filter(points_img_noise, sigma = _pred_radii*0.4)
                                        ...
                                        _point_r = np.array(np.arange(0,_pred_radii+20,sampling))
                                        _zeros_r = np.zeros(len(_point_r))
                                        _point_r = np.array([_point_r,_zeros_r])
                                        _centre_r = closest_node([len(points_img_noise_smooth)//2+1,len(points_img_noise_smooth)//2+1],peak_local_max(points_img_noise_smooth.T))
                                        ...
                                        for deg in np.arange(0,360,10):
                                            ...
                                            points_vals = sp.ndimage.map_coordinates(points_img_noise_smooth.T,
                                                                                     rot_point_r, 
                                                                                     order=3,
                                                                                     cval=0)
                                            points_vals = sp.ndimage.gaussian_filter1d(points_vals,sigma = _pred_radii*.4/sampling)
                                        ...
                                        return radii, radii_std, radii_r, radii_std_r, sigma, std
                                    
                                    pool = multiprocessing.Pool(8)
                                    _vals, _vals_sigma, _vals_r, _vals_sigma_r, sigma, std = zip(*pool.map(calc_fwhm_path, np.random.choice(range(len(path)),8)))
                                    ...
                                    _sigma.extend(list(sigma))
                                    _std.extend(list(std))
                            np.savez(
                                re.sub('_warped.pickle','_changes_noise2.npz',re.sub('matt_raw_warped_single_upsampled_seg','simulated_changes',file)),
