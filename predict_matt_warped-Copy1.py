...
with torch.no_grad():
    for i, pred_data in tqdm(enumerate(pred_loader)):
        pred_array = np.empty((num_evals,3,512,512,254))
        print(pred_data["image"].shape)
        if pred_data["image"].shape[4]==254:
            ...
            new_file_name = re.sub('matt_raw_warped','matt_preds_registered',data_dicts[i]["image"])
